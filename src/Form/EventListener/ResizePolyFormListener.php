...
class ResizePolyFormListener extends ResizeFormListener
{
    ...
    public function __construct(array $prototypes, array $options = array(), bool $allowAdd = true, bool $allowDelete = false, $typeFieldName = '_type', $indexProperty = null, bool $useTypesOptions = false)
    {
        ...
        foreach ($prototypes as $prototype) {
            ...
            ParagraphHelper::addTypeField($prototype, $type);
     ...
     */
    protected function getTypeForObject($object): string
    {
    ...
    public function preSetData(FormEvent $event)
    {
        ...
        foreach ($data as $name => $value) {
            ...
            $form->add($name, $type, array_replace(
                ['property_path' => '[' . $name . ']'],
                array_replace($options, ['block_prefix' => '_paragraph'])
            ));

            // Make sure the _type field is set on every row
            !$form->get($name)->has('_type') && ParagraphHelper::addTypeField($form->get($name), $type);
        }
    }
    ...
    public function preSubmit(FormEvent $event)
    {
        ...
        if ($this->allowAdd) {
            foreach ($data as $index => $value) {
                ...
                if (!$form->has($index)) {
                    $form->add($index, $type, array_replace(array(
                        'property_path' => '[' . $index . ']',
                    ), $this->getOptionsForType($type)));
                }

                // Make sure the _type field is set on every row
                !$form->get($index)->has('_type') && ParagraphHelper::addTypeField($form->get($index), $type);
            }
        }
